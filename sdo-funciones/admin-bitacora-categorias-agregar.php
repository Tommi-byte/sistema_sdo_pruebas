...
if (isset($_POST['addNomCategoria'], $_POST['idCategoria'])) {
    ...
    if(!empty($addNomCategoria) && !empty($idCategoria) && $idCategoria > 0){
        ...
        if($existeSubcategoria == 0){
            ...
            if ($res) {
                header('Location: ../sdo-administrador/bitacora-listado.php?exitoAgregarCategoria=true');
            } else {
                header('Location: ../sdo-administrador/bitacora-listado.php?errorAgregarCategoria=true');
            }
        }else{
            header('Location: ../sdo-administrador/bitacora-listado.php?existeCategoria=true');
        }
    }else{
        header('Location: ../sdo-administrador/bitacora-listado.php?errorAgregarCategoria=true');
    }
