...
if (isset($_POST['categoriaBitacora'], $_POST['observacionBitacora'], $_POST['tecnicoBitacora'])) {
    ...
    if (!empty($categoriaBitacora) && !empty($observacionBitacora)  && !empty($tecnicoBitacora) && $categoriaBitacora > 0 && $tecnicoBitacora > 0) {
        ...
        $query .=  "VALUES (NULL,'" . $observacionBitacora . "',NOW(),'" . $tecnicoBitacora . "'," . $categoriaBitacora . ")";
        ...
        if ($res) {
            header('Location: ../sdo-administrador/bitacora-listado.php?exitoAgregarObservacion=true');
        } else {
            header('Location: ../sdo-administrador/bitacora-listado.php?errorAgregarObservacion=true');
        }
    } else {
        header('Location: ../sdo-administrador/bitacora-listado.php?errorAgregarObservacion=true');
    }
