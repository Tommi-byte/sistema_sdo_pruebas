...
$headerExito = 'Location: ../' . $ruta . '/soliCompra_administracion.php?exitoAsignarNumInterno=true';
$headerError = 'Location: ../' . $ruta . '/soliCompra_administracion.php?errorAsignarNumInterno=true';
$headerExiste = 'Location: ../' . $ruta . '/soliCompra_administracion.php?existeAsignarNumInterno=true';
...
if (isset($_POST['addNumInterno'], $_POST['idSoliCompra'])) {
    ...
    $existeNumInterno = existeNumInternoPM($addNumInterno);

    if ($addNumInterno > 0 && $idSoliCompra > 0) {

        if ($existeNumInterno == 0) {

            $queryModificar = "UPDATE solicitudes_compra_pm SET numSoliCompra = '" . $addNumInterno . "', estadoSoliCompra = 'Recibida por Puesta en Marcha' WHERE idSoliCompra = " . $idSoliCompra;
